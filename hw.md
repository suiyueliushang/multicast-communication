...
此时在另一台电脑上登录，名字改为71117409李鑫：
![img](pic/chang_name.png)
![img](pic/chang_name_1.png)
可以看到改名成功  

下面改动默认存储地址  
![img](pic/chang_address.png)
![img](pic/chang_add1.png)

然后让他也加入225.0.0.100，端口也用8888：  
点击多播ip，可以看到成员列表：
分别为：
![img](pic/show1.png)
![img](pic/show2.png)
成功实现了成员发现，如果需要更新成员信息，点击左边栏中的显示按钮，就可以对当前所有的成员进行刷新；  

在点击多播ip的同时右边聊天框也出现了，这边可以看到收到的消息：现在让双方互发消息：
![img](pic/message1.png)
![img](pic/message2.png)
成功看到了对方发的消息，但是消息并不会自动刷新，需要点击对应的多播ip，才会重新刷新消息；
